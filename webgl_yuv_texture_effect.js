...
filter.initialize = function() {
  ...
  img_tx.upload(new Texture("textures/index.jpeg", true));   // à changer le nom de l'image pour loader une autre texutre
  pck_tx.pbo = false;
...
void main(void) {
  vec2 tx= vTextureCoord;   // coordonnées (x,y) du px d'entrée dans le sysème de coordonnées de la texture (entre 1.0 et 1.0)
...
function setupProgram(gl, vsSource, fsSource)
{
  ...
  return {
    ...
    uniformLocations: {
      ...
      txImg: gl.getUniformLocation(shaderProgram, 'imgTx'),
...
function drawScene(gl, programInfo, buffers) {
  ...
  gl.uniform1f(programInfo.uniformLocations.txImg, pck_tx.nb_textures);
